...
int main()
{
    //Calls the constructor without specifying parameters, so that default values are used
    Signal x;
    
    //Fills the signal values with all values of 1
    x.setConstant(1);
    ...
    //Copies the first signal to the second signal using the overloaded = operator
    box = x;
