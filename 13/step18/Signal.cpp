...
void Signal::fillSignalArray(double amp, double freq, double phase)
{
    for (unsigned int i = 0; i < numSample; i++)
    {
        signal[i] = amp * cos((freq * (time[i] - initialTime) + phase));
    }
...
//Member Function for the critically damped case 
void Signal::fillCritDamp(double slope, double time2, double b)
{
    ...
    //Making the expression of the equation to be what the instructions gave us
    expression = getFunction() + "(t) = " + textNum(slope) + " (t + " + textNum(fabs(time2)) + ") - " + textNum(fabs(b));
}
