...
//Accessor Function that gives the solution to the Differential Equation 
string Lhdeccic::getSolution() const
{
    return diffLabel + "(t) = " + solution.getExpression();
}

//Member Function that displays the solution to an output file
void Lhdeccic::outFile2(ofstream& outFile) const
{
    outFile << "Solution\n--------\n";
    outFile << getSolution() << endl << endl;
    solution.outFile();
}
